...
exports.dashboard = async (req, res) => {
    ...
    var feedback = await Feedback.findAll();//to find all fooddata feedbacks

    var counts = {};
    counts.breakfast = await Food.count({ where: { meal_Type: 1 } });
    counts.lunch = await Food.count({ where: { meal_Type: 2 } });
    counts.dinner = await Food.count({ where: { meal_Type: 3 } });
...
exports.SaveAdminProfile = async (req, res) => {
    ...
    try {
        ...
        var admindata = await User.findOne({ where: { email_id: req.session.email } });// to find all data of Admin Data
